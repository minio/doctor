...
(function() {
  $(document).on('page:change', function() {
    if ($('.form-group__field--color')[0]) {
      ...
      return $('.form-group__field--color').mask('ASSSSSS', {
        'translation': {
          A: {
            pattern: /['#']/
          },
...
(function() {
  ...
  $(document).on('page:change', function() {
    var $body, $pathname, clipboard, closeImgPreview, url;
    $body = $('body');
    if ($('.main__sidebar--scroll')[0]) {
      $pathname = window.location.pathname;
      $('.main__sidebar--scroll').scrollTop($('a[href="' + $pathname + '"]').offset().top - 82);
    }
    if ($('.header__menu__search')[0]) {
      $('body').on('click', '.header__menu__search > a', function(e) {
        e.preventDefault();
        $('.header__search').fadeIn(300);
        $('.header__search input[type=text]').focus();
      });
      $('body').on('click', '.header__search__close', function() {
    ...
    $body.on('click', '.backdrop--dropdown', function(e) {
      e.preventDefault();
      $(this).remove();
      $('.dropdown--open').removeClass('dropdown--open');
    });
    closeImgPreview = function() {
      var $closeTarget, callBack;
      $closeTarget = $('.img-preview');
      $closeTarget.fadeOut(200);
      callBack = function() {
        return $closeTarget.remove();
      };
      setTimeout(callBack, 250);
    };
    $body.on('click', '.main__page *:not("a") > img:not(".img-preview__img")', function() {
      var $imgSrc, $imgWrap;
      $imgSrc = $(this).attr('src');
      $imgWrap = '<div class="img-preview">' + '<div class="img-preview__close img-preview__close--back"></div>' + '<i class="img-preview__close img-preview__close--icon fa fa-close"></i>' + '<img class="img-preview__img" src=' + $imgSrc + ' alt="">' + '</div>';
      $body.append($imgWrap);
    });
    $body.on('click', '.img-preview__close', function() {
      closeImgPreview();
    });
    ...
    $(document).one('page:receive', function() {
      $('.page-loader').remove();
    });
    $body.on('click', '.header__trigger', function() {
      var $sidebarBckdrop;
      $sidebarBckdrop = '<div class="backdrop backdrop--sidebar" />';
      $body.addClass('prevent-scroll').append($sidebarBckdrop);
      $('.main__sidebar').addClass('toggled');
    });
    $body.on('click', '.backdrop--sidebar', function() {
    ...
    $('.notify').addClass('notify--show').delay(5000).queue(function() {
    ...
    $('pre').each(function() {
      if ($(this).find('.copy-to-clipboard').length === 0) {
        return $(this).prepend('<span class="copy-to-clipboard">Copy</span>');
      }
    });
    ...
    if ($('.action-header__item--tabs')[0]) {
      ...
      $('.action-header__item--tabs > li > a').on('shown.bs.tab', function(e) {
